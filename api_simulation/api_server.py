...
def fetch_permits(count=40, offset=0):
    ...
    Args:
        count (int):
            Number of permits to retrieve this call.
        offset (int):
            Starting index for the batch, so repeated calls can fetch
            genuinely new, non-overlapping permits instead of always
            re-fetching the same first N.
